...
class ModuleManagerListener extends AbstractModuleActionListener
{
    ...
    {
        return 'Juggernaut\\OpenEMR\\Modules\\GoogleCalendar\\';
    }
    ...
    {
        $logMessage = 'Remove credentials and tokens from system'; // Initialize an empty string to store log messages
        $sql = "DROP TABLE `module_google_calendar_credentials`";
        $status = sqlQuery($sql);
        $this->logger->info($logMessage . ' ' . text($status));
        $sql = "DROP TABLE `module_google_calendar_token`";
        $status = sqlQuery($sql);
        $this->logger->info($logMessage . ' ' . text($status));
        return $currentActionStatus;
    }
