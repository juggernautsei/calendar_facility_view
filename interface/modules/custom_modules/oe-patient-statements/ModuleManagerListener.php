...
/*
    ...
    $classLoader->registerNamespaceIfNotExists("Juggernaut\\Module\\PatientStatement\\", __DIR__ . DIRECTORY_SEPARATOR . 'src');
*/
...
class ModuleManagerListener extends AbstractModuleActionListener
{
    ...
    {
        return 'Juggernaut\\Module\\PatientStatements\\';
    }
    ...
    private function enable($modId, $currentActionStatus): mixed
    {
        ...
        error_log("Enabling BALANCE_REMINDERS");
        $sql = "UPDATE `background_services` SET `active` = 1 WHERE `name` = 'BALANCE_REMINDERS'";
        sqlQuery($sql);
        return $currentActionStatus;
    }
    ...
    {
        // Return the current action status from Module Manager in case of error from its action.
        error_log("Disabling BALANCE_REMINDERS");
        $sql = "UPDATE `background_services` SET `active` = 0 WHERE `name` = 'BALANCE_REMINDERS'";
        sqlQuery($sql);
        return $currentActionStatus;
    }
    ...
    {
        $sql = "DELETE FROM `background_services` WHERE `name` = 'BALANCE_REMINDERS'";
        sqlQuery($sql);
        $sql = "DROP TABLE `module_patient_statement_settings`";
        sqlQuery($sql);
        return $currentActionStatus;
    }
    ...
    private function help_requested($modId, $currentActionStatus): mixed
    {
        ...
        if (file_exists(__DIR__ . '/help.php')) {
