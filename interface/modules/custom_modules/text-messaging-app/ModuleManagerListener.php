...
class ModuleManagerListener extends AbstractModuleActionListener
{
    ...
    {
        /**
         * This task is handled in the module and is not automatically done by the module
         */
    ...
    {
        $logMessage = 'If active, deactivate the background services tasks have been disabled';
        // Unregister background services
        $sql = "UPDATE `background_services` SET `active` = '0' WHERE `name` = ? OR `name` = ?";
        $status = sqlQuery($sql, array('SMS_REMINDERS', 'Provider_Reminders'));
    ...
    {
        $logMessage = 'Texting Background tasks have been removed'; // Initialize an empty string to store log messages
