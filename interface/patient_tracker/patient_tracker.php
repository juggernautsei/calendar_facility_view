...
require_once "insurance.inc.php";
...
<body>
    ...
    ?>
		...
		<div id="pull_kiosk_right" class="text-right">
			...
			<td class="dehead text-center  d-sm-table-cell ">
			    <?php
			    if ($_SESSION['site_id'] == 'kirnplasticsurgery') {
			    echo xlt('Appt Status') . '/' . xlt('Physician');
			    } else {
			...
			<td class="dehead text-center " name="kiosk_hide">
			     <?php
				if ($_SESSION['site_id'] == 'kirnplasticsurgery') {
				    echo xlt('Room');
				} else {
			...
			<?php
			 if ($_SESSION['site_id'] == 'reencuentro') { ?>
			    <td class="dehead center " name="kiosk_hide">
				<?php echo xlt('Identified Therapist'); ?>
			    </td>
			    <?php
			    }
			?>
			...
			<?php } ?>
			<td class="detail text-center" name="kiosk_hide" style="color: black">
				<?php
			...
			<td style="width: 150px; color: black;">
			    <?php
				echo "C " . $appointment['phone_cell'] . "<br> H " . $appointment['phone_home'];
			    ?>
			</td>
			<?php /*if ($GLOBALS['ptkr_show_encounter']) { ?> removed by Sherwin to make columns line up
			    <td class="detail text-center" name="kiosk_hide" style="color: black">
				<?php
				if ($appt_enc != 0) {
				    echo text($appt_enc);
			...
			<td class="detail text-center ">
				...
				echo text(getListItemTitle("apptstat", $status)) . "<br> " . text(getListItemTitle('patient_flow_board_rooms', $appt_room));
			    }
			    ?>
			    </a>
			</td>
			...
			<td class="detail text-center" name="kiosk_hide" style="color: black">
			     <?php
			     if ($_SESSION['site_id'] == 'kirnplasticsurgery') {
				 $r = sqlQuery("SELECT `fname`, `lname`  FROM `users` WHERE `id` = ?", array($appointment['pc_aid']));
				 echo $r['lname'];
			     } else {
			...
			<?php
			 if ($_SESSION['site_id'] == 'reencuentro') { ?>
			    <td class="dehead center " name="kiosk_hide">
				<?php
				 $therapist = sqlQuery("SELECT IdentTx FROM `patient_data` WHERE pid = ?",[$appt_pid]);
				 echo text($therapist['IdentTx']);
				?>
			    </td>
			    <?php
			    }
