...
class PostCalendarTwigExtensions  extends AbstractExtension
{
     ...
     * @param int $providerid Provider ID to filter events for
     * @param int $calStartMin Calendar start time in minutes
     * @param int $calEndMin Calendar end time in minutes
     * @param int $timeslotHeightVal Height value for time slots 
     * @param string $timeslotHeightUnit Unit for time slot height (px, em, etc)
     * @param int $interval Time slot interval in minutes
     * @return string HTML content for events
     */
    public function PrintEvents($events, $providerid, $calStartMin, $calEndMin, $timeslotHeightVal, $timeslotHeightUnit, $interval)
    {
        global $GLOBALS;
        ...
        // Loop through events and filter by provider
        foreach ($events as $event) {
            // Skip events that don't belong to this provider
            if ($event['aid'] != $providerid) {
                continue;
            }
            ...
            $starth = substr($event['startTime'], 0, 2);
            $startm = substr($event['startTime'], 3, 2);
            $starttimeh = $starth * 60 + $startm;
            
            $endh = substr($event['endTime'], 0, 2);
            $endm = substr($event['endTime'], 3, 2);
            $endtimeh = $endh * 60 + $endm;
            
            // Skip if event is outside the displayed time range
            if ($endtimeh < $calStartMin || $starttimeh > $calEndMin) {
                continue;
            }
            
            // Adjust display for events that start before the displayed time
            if ($starttimeh < $calStartMin) {
                $starttimeh = $calStartMin;
            }
            
            // Calculate event duration and display position
            $durationh = $endtimeh - $starttimeh;
            $heightPx = round($timeslotHeightVal * $durationh / $interval);
            
            // Get event category for styling
            $catid = $event['catid'];
            $comment = $event['hometext'];
            $eventStatus = $event['apptstatus'];
            
            // Determine the event's CSS class
            $eventClass = "event_appointment";
            if (strstr($eventStatus, 'IN')) {
                $eventClass = "event_in";
            } else if (strstr($eventStatus, 'NO')) {
                $eventClass = "event_noshow";
            } else if (strstr($eventStatus, 'OUT')) {
                $eventClass = "event_out";
            } else if (strstr($eventStatus, 'RESERVED')) {
                $eventClass = "event_reserved";
            }

            // Format the event display content
            $content = "";
            
            // Time
            $content .= text(substr($event['startTime'], 0, 5)) . "-" . text(substr($event['endTime'], 0, 5));
            ...
            $output .= "<div class='{$eventClass}' style='height: {$heightPx}{$timeslotHeightUnit};'>";
            $output .= $content;
            $output .= "</div>";
        }
